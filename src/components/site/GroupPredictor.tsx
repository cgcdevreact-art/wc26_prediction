...
function getGroupMatchDetails(group: string, suffix: number) {
  ...
  if (suffix === 1) {
    date = "Jun 12";
    time = groupOffset % 2 === 0 ? "12:30 AM" : "03:30 PM";
    matchNumber = groupOffset * 2 + 1;
    venue = ["Boston", "Los Angeles", "New York/NJ", "Dallas", "Miami"][groupOffset % 5];
  } else if (suffix === 2) {
    date = "Jun 12";
    time = groupOffset % 2 === 0 ? "07:30 AM" : "08:30 PM";
    matchNumber = groupOffset * 2 + 2;
    venue = ["Toronto", "Mexico City", "Vancouver", "Seattle", "Atlanta"][groupOffset % 5];
  } else if (suffix === 3) {
    date = "Jun 18";
    time = groupOffset % 2 === 0 ? "09:30 PM" : "01:30 PM";
    matchNumber = 24 + groupOffset * 2 + 1;
    venue = ["Houston", "Kansas City", "San Francisco", "Monterrey", "Philadelphia"][groupOffset % 5];
  } else if (suffix === 4) {
    date = "Jun 19";
    time = groupOffset % 2 === 0 ? "06:30 AM" : "05:30 PM";
    matchNumber = 24 + groupOffset * 2 + 2;
    venue = ["Dallas", "Miami", "Boston", "Los Angeles", "New York/NJ"][groupOffset % 5];
  } else if (suffix === 5) {
    date = "Jun 25";
    time = "06:30 AM";
    matchNumber = 48 + groupOffset * 2 + 1;
    venue = ["Mexico City", "Atlanta", "Seattle", "Toronto", "Vancouver"][groupOffset % 5];
  } else if (suffix === 6) {
    date = "Jun 25";
    time = "06:30 AM";
    matchNumber = 48 + groupOffset * 2 + 2;
    venue = ["Monterrey", "Philadelphia", "Houston", "Kansas City", "San Francisco"][groupOffset % 5];
  }

  return { date, time, matchNumber, venue };
}
...
export function GroupPredictor({ defaultTab = "group", onlyKnockout = false, fullWidth = false }: GroupPredictorProps) {
  ...
  return (
    <div className={`mx-auto px-2 py-8 md:px-4 transition-all duration-300 ${fullWidth || activeTab === "knockout" ? "max-w-none w-full" : "max-w-7xl"}`}>
      ...
      <div className="w-full mb-8 rounded-3xl p-6 md:p-8 border border-white/5 bg-[#121623] shadow-lg animate-fade-in">
        <div className="flex items-center justify-between mb-8">
          <div>
            <h3 className="text-[11px] font-bold uppercase tracking-widest text-muted-foreground/80 mb-1">
              Live win probability
            </h3>
            <h2 className="font-display text-2xl font-bold text-white">Top contenders</h2>
          </div>
          <div className="flex h-12 w-12 items-center justify-center rounded-full bg-gradient-to-br from-yellow-600/40 to-yellow-900/40 border border-yellow-600/30">
          ...
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={topTeamsData} margin={{ top: 20, right: 0, left: 0, bottom: 0 }}>
              ...
              <Tooltip 
                ...
                contentStyle={{ background: "#0f172a", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 12, fontSize: 13, boxShadow: "0 10px 40px -10px rgba(0,0,0,0.5)" }}
                formatter={(value: any, name: any, props: any) => {
                  const isElim = props?.payload?.isEliminated;
                  return [`${value}%${isElim ? ' (Out)' : ''}`, "Win Probability"];
                }}
                labelStyle={{ color: "#6EE7B7", fontWeight: "bold", marginBottom: 6 }}
        ...
        <div className="mt-8 grid grid-cols-2 md:grid-cols-4 gap-4">
          {topTeamsData.slice(0, 4).map((team) => (
            <div 
              key={team.code} 
              className="flex flex-col items-center justify-center bg-white/5 border border-white/10 rounded-2xl py-4 shadow-glass transition-all animate-fade-in"
            >
              ...
              <span className="text-sm font-bold text-[#6EE7B7] mt-1 flex items-center gap-1.5">
            ...
            <div className="space-y-8 animate-fade-in">
              {(() => {
                const renderCompactGroupCard = (groupName: string) => {
                  const groupStandings = standings[groupName] || [];
                  return (
                    <div key={groupName} className="w-56 bg-black/40 border border-white/5 rounded-2xl p-3 shrink-0 flex flex-col justify-between hover:border-white/10 transition duration-200">
                      <div className="text-center font-display font-bold text-xs text-gradient pb-1.5 border-b border-white/5 mb-1.5">
                        Group {groupName}
                      </div>
                ...
                return (
                  <>
                    <div className="flex justify-center">
                      <button
                        onClick={handleAiPredictKnockoutsWithCredits}
                        className="flex items-center gap-2 rounded-lg bg-white/5 border border-white/10 px-6 py-2.5 text-sm font-semibold hover:bg-white/10 transition text-neon shadow-neon"
                      >
                        <Sparkles className="h-4 w-4" />
                    ...
                    <div className="w-full select-none border border-white/5 rounded-3xl bg-black/20 p-2 md:p-4">
                      <div className="flex gap-4 items-stretch w-full px-2 lg:px-4 py-6 overflow-x-auto scrollbar-custom">
                        ...
                        <div className="flex flex-col w-56 min-w-[224px] shrink-0 gap-3">
                          ...
                          <div className="flex-1 flex flex-col justify-around gap-24 py-16">
                        ...
                        <div className="flex flex-col w-56 min-w-[224px] shrink-0 gap-3">
                          ...
                          <div className="flex-1 flex flex-col justify-around gap-48 py-24">
                        ...
                        <div className="flex flex-col w-64 min-w-[256px] shrink-0 gap-3 justify-center">
                          ...
                          <div className="text-center mb-4">
                            {koWinners.final[0] ? (
                              <div className="glass-strong rounded-3xl p-6 border border-neon/50 shadow-neon flex flex-col items-center gap-4 text-center animate-float bg-black/40">
                              ...
                              <div className="glass rounded-3xl p-6 border border-white/5 flex flex-col items-center gap-3 text-center opacity-60">
...
function KnockoutMatchCard({
  ...
  if (isLocked) {
    return (
      <div className="glass rounded-xl p-3 border border-white/5 opacity-40 flex flex-col justify-center min-h-[110px] text-center shadow-glass">
  ...
  return (
    <div className="glass-strong rounded-xl p-3 border border-white/5 hover:border-neon/20 transition flex flex-col justify-between min-h-[110px] shadow-glass bg-black/30">
      <div
        onClick={onSimulateClick}
        title="Simulate 1v1"
        className="flex items-center justify-between text-[10px] uppercase font-bold tracking-wider text-muted-foreground border-b border-white/5 pb-1.5 mb-2 cursor-pointer hover:text-neon group/hdr transition duration-200"
      >
