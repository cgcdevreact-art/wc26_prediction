...
export default function CommissionerUsersPage() {
  ...
  const fetchUsers = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/commissioner/users?${params}`);
  ...
  const handleCreateUser = async (e: React.FormEvent) => {
    ...
    setCreating(true);
    try {
      const res = await fetch("/api/commissioner/users", {
        method: "POST",
  ...
  return (
    <>
      <CommissionerHeader title="Users" description={`${total} registered users`} />
