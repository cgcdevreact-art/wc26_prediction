...
export default function CommissionerCustomPollsPage() {
  ...
  const loadPolls = async () => {
    setLoading(true);
    try {
      const res = await fetch("/api/commissioner/custom-polls");
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const url = editingPollId ? `/api/commissioner/custom-polls/${editingPollId}` : "/api/commissioner/custom-polls";
  ...
  const handleArchive = async (pollId: string) => {
    ...
    try {
      const res = await fetch(`/api/commissioner/custom-polls/${pollId}`, {
        method: "DELETE",
      });
      const data = await res.json();
