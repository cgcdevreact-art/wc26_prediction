...
function getTeamIdFromCode(code) {
...
async function main() {
  console.log("Rolling back old winner prediction data...");
  const deletePredicts = await prisma.prediction.deleteMany({
    where: { type: "VOTE_CHAMPION" }
  });
  console.log(`Deleted ${deletePredicts.count} predictions.`);
  ...
  console.log(`Deleted ${deleteUsers.count} seed users.`);

  const allTeams = [...teams, "OTH"];
  
  const data = rawData.map(pt => {
    let sum = 0;
    teams.forEach(t => sum += pt[t]);
  ...
  for (let j = 0; j < data.length; j++) {
    ...
    if (j === 0) {
      const votes = {};
      allTeams.forEach(t => {
        votes[t] = Math.round(pt[t] * (currentTotal / 100));
      });
      result.push({
        date: pt.date,
        total: currentTotal,
  ...
  console.log("Seeding calculated daily vote increments...");
  ...
  console.log(`Prepared ${userBatch.length} votes to seed.`);
  ...
  for (let offset = 0; offset < userBatch.length; offset += BATCH_SIZE) {
    ...
    console.log(`Seeded offset ${offset + uBatch.length} / ${userBatch.length}`);
  }
